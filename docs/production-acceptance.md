...
| 20 | Installer, updater, and migrations are tested | External gate | Packaging, signed verification, safe extraction, atomic two-binary install/rollback, configuration migration/backup, and local tests exist. Exercise the published signed artifact end to end. |
| 21 | Cross-platform integration tests pass | External gate | CI targets macOS, Linux, and Windows. The upstream workflow and signed release matrix must run successfully. |
...
The offline runtime is not yet release-approved. Exercise criteria 20 and 21 against an actual
published release, then supply credentials locally through `purrcode credential set` to execute
criteria 1, 4, 5, and 6. Secrets must never be pasted into chat or configuration.
