...
impl ModelRoute {
    ...
    pub(crate) async fn structured<T: serde::de::DeserializeOwned>(
        ...
        let value = self.provider.structured(request, schema).await?;
        serde_json::from_value(value).map_err(|error| AlignmentError::Decode(format!("{error}")))
    }
}
