...
use purrcode_runtime_core::{ActionId, DeleteFileAction, ProposedAction, WriteFileAction};
            ...
            ProposedAction::Command(command) => {
                ...
                if constraints.network {
                    return Err(ExecutionError::UnsupportedConstraint(
                        "network-enabled execution is not implemented by the process backend"
                            .into(),
                    ));
                }
                let (mut process, sandbox_level, sandbox_backend) =
                    sandboxed_command(command, constraints)?;
                process
                ...
                let stdout_task =
                    tokio::spawn(async move { read_bounded(&mut stdout, limit).await });
                let stderr_task =
                    tokio::spawn(async move { read_bounded(&mut stderr, limit).await });
...
mod tests {
    use super::*;
    use chrono::Utc;
    use purrcode_runtime_core::{ActionConstraints, ApprovalAuthority, Authorization, SessionId};
